...
function assemble_squaremesh_FE_matrix(el_mat::Array{Float64, 2},
    ...
    l2 = zeros(UInt64, n_order1 * dof1, nT)
    r2 = zeros(UInt64, n_order2 * dof2, nT)
...
function assemble1d_squaremesh_FE_matrix(el_mat::Array{Float64, 2},
    ...
    l2 = zeros(UInt64, n_order2 * dof2, nT)
    v = dof2 * ((1:n_order2).-1)
    Threads.@threads for i = 1:elements1d_N
        k = Threads.threadid()
        
        for j = 1:dof2
            l2[v .+ j, k] = @. dof1 * (elements1d[1:n_order2, i] - 1) + j
        end
        for j = 1:dof2
            r2[v .+ j, k] = @. dof2 * (elements1d[1:n_order2, i] - 1) + j
        end
        MM[k][l2[:,k], r2[:,k]] +=  el_mat
    end
    M = MM[1]
